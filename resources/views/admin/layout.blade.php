...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .siswa-info { font-size:.8rem; color:#666; margin-bottom:.5rem; }
        ...
        .btn-update-toggle { padding:.45rem 1rem; background:#e8f5e9; border:1.5px solid #2e7d32; border-radius:8px; color:#2e7d32; font-family:inherit; font-size:.8rem; font-weight:600; cursor:pointer; }
        .update-panel { display:none; background:#f9fdf9; border:1.5px solid #e0e0e0; border-radius:10px; padding:1rem; margin-top:.75rem; }
        .update-panel.open { display:block; }
        .btn-save { padding:.6rem 1.5rem; background:#2e7d32; color:white; border:none; border-radius:8px; font-family:inherit; font-size:.85rem; font-weight:600; cursor:pointer; }
...
<div class="header">
    <div class="header-left">
        ...
        <h1>Aspirasi Siswa - Admin</h1>
    </div>
    <div class="header-right">
        <div class="user-info">
            <div class="name">{{ Session::get('admin.username') }}</div>
        </div>
...
<div class="navbar">
    <a href="{{ route('admin.dashboard') }}" class="nav-tab {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}"><i class="fas fa-home"></i> Dashboard</a>
    <a href="{{ route('admin.masuk') }}" class="nav-tab {{ request()->routeIs('admin.masuk') ? 'active' : '' }}"><i class="fas fa-inbox"></i> Aspirasi Masuk</a>
    <a href="{{ route('admin.semua') }}" class="nav-tab {{ request()->routeIs('admin.semua') ? 'active' : '' }}"><i class="fas fa-history"></i> Semua Aspirasi</a>
    <a href="{{ route('admin.kategori') }}" class="nav-tab {{ request()->routeIs('admin.kategori') ? 'active' : '' }}"><i class="fas fa-tags"></i> Kategori</a>
</div>
<div class="container">
    @if(session('success'))
...
function toggleRating(id, status) {
    const group = document.getElementById('rating-group-' + id);
    group.style.display = ['Proses', 'Selesai'].includes(status) ? 'block' : 'none';
    if (!['Proses', 'Selesai'].includes(status)) {
        document.getElementById('rating-val-' + id).value = '';
        document.querySelectorAll('#stars-' + id + ' .star').forEach(s => s.style.color = '#d1d5db');
    }
}
function setRating(id, val) {
    document.getElementById('rating-val-' + id).value = val;
    document.querySelectorAll('#stars-' + id + ' .star').forEach(star => {
